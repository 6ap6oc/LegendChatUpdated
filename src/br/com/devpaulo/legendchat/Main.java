...
public class Main extends JavaPlugin implements PluginMessageListener {
    ...
    public void onEnable() {
		log.info("Legendchat Updated(V"+getDescription().getVersion()+") - Author: SubZero0 (Original code) Sentinel with Coffee dream community");
		log.warning("Данный плагин восстановлен и адаптирован под версию Spigot/Paper 1.14 ");
		log.warning("и может содержать в себе ошибки!");
		log.warning("При обнаружении ошибок просьба сообщать https://vk.com/coffeemc в сообщения группы");
		log.warning("Or this profile - https://www.spigotmc.org/members/6ap6oc.618591/");
		...
		try {if(!new File(getDataFolder(),"language"+File.separator+"language_ru.yml").exists()) {saveResource("language"+File.separator+"language_ru.yml",false);getLogger().info("Saved language_ru.yml");}}
		catch(Exception e) {}
