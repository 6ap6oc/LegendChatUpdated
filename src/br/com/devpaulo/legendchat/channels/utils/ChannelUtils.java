...
public class ChannelUtils {
	...
	public static void realMessage(Channel c, Player sender, String message, String bukkit_format, boolean cancelled) {
		...
		ChatMessageEvent CHAT_EVENT = new ChatMessageEvent(c, sender, message,Legendchat.format(c.getFormat()), c.getFormat(), bukkit_format, recipients, tags, cancelled);
		Bukkit.getScheduler().runTaskLater(Legendchat.getPlugin(), () -> {
			Bukkit.getPluginManager().callEvent(CHAT_EVENT);
		}, 10);
		
		if(CHAT_EVENT.isCancelled())
			return;
		sender = CHAT_EVENT.getSender();
		message = CHAT_EVENT.getMessage();
		if(Legendchat.isCensorActive())
			message = Legendchat.getCensorManager().censorFunction(message);
		String completa = CHAT_EVENT.getFormat();
		if(Legendchat.blockRepeatedTags()) {
			if(CHAT_EVENT.getTags().contains("prefix")&&CHAT_EVENT.getTags().contains("groupprefix"))
				if(CHAT_EVENT.getTagValue("prefix").equals(CHAT_EVENT.getTagValue("groupprefix")))
					CHAT_EVENT.setTagValue("prefix", "");
			if(CHAT_EVENT.getTags().contains("suffix")&&CHAT_EVENT.getTags().contains("groupsuffix"))
				if(CHAT_EVENT.getTagValue("suffix").equals(CHAT_EVENT.getTagValue("groupsuffix")))
					CHAT_EVENT.setTagValue("suffix", "");
		}
		for(String n : CHAT_EVENT.getTags())
			completa = completa.replace("{"+n+"}", ChatColor.translateAlternateColorCodes('&', CHAT_EVENT.getTagValue(n)));
		completa = completa.replace("{msg}", translateAlternateChatColorsWithPermission(sender, message));
		
		for(Player p : CHAT_EVENT.getRecipients())
		...
		if(c.getMaxDistance()!=0)
			if(Legendchat.showNoOneHearsYou()) {
				boolean show = false;
				if(CHAT_EVENT.getRecipients().size()==0)
					show=true;
				else if(CHAT_EVENT.getRecipients().size()==1 && CHAT_EVENT.getRecipients().contains(sender))
					show=true;
				else {
					show=true;
					for(Player p : CHAT_EVENT.getRecipients())
		...
		for(Player p : Legendchat.getPlayerManager().getOnlineSpys())
			if(!CHAT_EVENT.getRecipients().contains(p))
			...
			if(Legendchat.isBungeecordActive()) {
				if(Legendchat.getBungeecordChannel()==((BungeecordChannel)c)) {
					...
					try {
						...
						for(String tag_packet : CHAT_EVENT.getTags())
							tags_packet.put(tag_packet,CHAT_EVENT.getTagValue(tag_packet));
