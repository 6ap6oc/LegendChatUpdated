...
//		InputStream is = plugin.getResource(("config_template.yml").replace('\\', '/'));
		...
		File file = new File (plugin.getDataFolder()+File.separator+"config_template.yml");
		...
		for(String n : config.getConfigurationSection("").getKeys(true))
			if(!has(n))
				set(n, config.get(n));
		if(updConfig)
			plugin.saveConfig();
	...
	public boolean updateAndLoadLanguage(String language) {
		/*
		plugin.saveResource(plugin.getDataFolder()+"language"+File.separator+"language_br.yml", false);
		plugin.saveResource(plugin.getDataFolder()+"language"+File.separator+"language_cn.yml", false);
		plugin.saveResource(plugin.getDataFolder()+"language"+File.separator+"language_en.yml", false);
		*/
		File file = new File(plugin.getDataFolder(),"language"+File.separator+"language_"+language+".yml");
		
		MessageManager manager = Legendchat.getMessageManager();
		
		manager.registerLanguageFile(file);
		manager.loadMessages(file);
/*		
		InputStream is = null;
...
*/		
		...
		for(String n : config.getConfigurationSection("").getKeys(false))
			if(!manager.hasMessage(n))
				addMessage(manager, n, config.getString(n));
		if(updLang)
			manager.loadMessages(file);
		return updLang;
	}
